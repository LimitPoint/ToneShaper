...
class PlotAudioObservable: ObservableObject  {
    ...
    func downsampleAudioToPlot(asset:AVAsset, downsampleRateSeconds:Int?) {
        ...
        asset.loadTracks(withMediaType: .audio) { [weak self] audioTracks, error in
            ...
            if let audioTrack = audioTracks?.first { 
                ...
                self.downsampleAudio = DownsampleAudio(noiseFloor: Double(self.noiseFloor), antialias: pathAntialias)
                
                self.downsampleAudio?.run(asset: asset, audioTrack: audioTrack, downsampleCount: self.downsampleCount(), downsampleRateSeconds: downsampleRateSeconds) { audioSamples in
