...
class DownsampleAudio {
    ...
    func audioReader(asset:AVAsset, audioTrack:AVAssetTrack, outputSettings: [String : Any]?) -> (audioTrack:AVAssetTrack?, audioReader:AVAssetReader?, audioReaderOutput:AVAssetReaderTrackOutput?) {
    ...
    func readAndDownsampleAudioSamples(asset:AVAsset, audioTrack:AVAssetTrack, downsampleCount:Int, completion: @escaping ([Double]?) -> ()) {
        
        let (_, reader, readerOutput) = self.audioReader(asset:asset, audioTrack: audioTrack, outputSettings: kPAAudioReaderSettings)
        ...
        let totalSampleCount = asset.audioBufferAndSampleCounts(audioTrack: audioTrack, kPAAudioReaderSettings).sampleCount
    ...
    func readAndDownsampleAudioSamples(asset:AVAsset, audioTrack:AVAssetTrack, downsampleCount:Int, downsampleRateSeconds:Int, completion: @escaping ([Double]?) -> ()) {
        ...
        guard let sampleBuffer = asset.audioSampleBuffer(audioTrack: audioTrack, outputSettings:kPAAudioReaderSettings),
    ...
    func run(asset:AVAsset, audioTrack: AVAssetTrack, downsampleCount:Int, downsampleRateSeconds:Int?, completion: @escaping ([Double]?) -> ()) {
        downsampleAudioQueue.async { [weak self] in
            ...
            if let downsampleRateSeconds = downsampleRateSeconds {
                self.readAndDownsampleAudioSamples(asset: asset, audioTrack: audioTrack, downsampleCount: downsampleCount, downsampleRateSeconds: downsampleRateSeconds) { audioSamples in
                ...
                self.readAndDownsampleAudioSamples(asset: asset, audioTrack: audioTrack, downsampleCount: downsampleCount) { audioSamples in
